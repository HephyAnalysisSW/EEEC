...
filename = "EEEC_PP_13000_1725_804_HARDPT300to900_MPIon_HADon_JETDEF.root"
# ptbins = ["500to525", "525to550", "550to575", "575to600", "600to625", "625to650", "650to675", "675to700"]
ptbins = ["500to525"]

jetdefs = ["default", "R08", "R12", "CA"]
colors = {
    "R08": 93,
    "default": 1,
    "R12": 69,
...
legnames = {
    "R08":     "anti-k_{T}, R = 0.8",
    "default": "anti-k_{T}, R = 1.0",
    "R12":     "anti-k_{T}, R = 1.2",
    "CA":      "Cambridge-Aachen, R = 1.0",
}
...
for ptbin in ptbins:
    ...
    for jet in jetdefs:
        replaceString = "" if jet == "default" else "_"+jet
        hist3D = getObjFromFile(filedir+filename.replace("_JETDEF", replaceString), "EEEC1_jetpt"+ptbin)
        logger.info( "Make 3D -> 1D projection for jet definition: %s", jet)
        hists[jet] = project3Dto1D(hist3D, 0.003, 0.01, jet+"_"+ptbin, jet+"_"+ptbin)
        # hists_stat[jet] = hists[jet].Clone(hists[jet].GetName()+"_stat")
    ...
    hists["default"].SetTitle("")
    hists["default"].GetXaxis().SetTitle("(#Delta R^{2}_{max}+#Delta R^{2}_{med})/2")
    hists["default"].GetYaxis().SetTitle("Weighted triplets")
    hists["default"].Draw("HIST")
    leg = ROOT.TLegend(.6, .6, .85, .85)

    for jet in jetdefs:
        hists[jet].SetLineWidth(2)
        hists[jet].SetLineColor(colors[jet])
        hists_stat[jet].SetFillColorAlpha(colors[jet], 0.3)
        hists_stat[jet].SetMarkerSize(0.0)
        # hists_stat[jet].Draw("E3 SAME")
        hists[jet].Draw("HIST SAME")
        leg.AddEntry(hists[jet], legnames[jet], "l")
    leg.Draw()

    c.Print(plot_directory+"/EEEC_jetDefs/EEEC_jetDefs_jetpt"+ptbin+".pdf")
